...
// ─── OpenJarvis Proxy Bridge ──────────────────────────────────
// Allows the dashboard server (3001) to query Jarvis (8000) server-side

const JARVIS_API = 'http://127.0.0.1:8000';

app.get('/api/jarvis/health', async (req, res) => {
    try {
        const r = await fetch(`${JARVIS_API}/v1/models`);
        if (r.ok) {
            const data = await r.json();
            return res.json({ online: true, models: data.data || [] });
        }
...
app.post('/api/jarvis/chat', async (req, res) => {
    try {
        ...
        const r = await fetch(`${JARVIS_API}/v1/chat/completions`, {
            method: 'POST',
        ...
        if (!r.ok) {
            ...
            return res.status(r.status).json({ error: errData.error?.message || 'Jarvis API Error' });
        }

        const data = await r.json();
        res.json({ message: data.choices[0].message.content, model: data.model, usage: data.usage });
    } catch (err) {
        console.error('[Jarvis Bridge] Error:', err.message);
        res.status(503).json({ error: 'OpenJarvis server not reachable. Run: uv run jarvis start' });
    }
});
...
app.get('/api/jarvis/status', async (req, res) => {
    try {
        ...
        res.json({ online: false, error: 'Jarvis daemon not running' });
    }
});

// ─── Auto-Delegation Pipeline: Bart → Jarvis → Antigravity ───
// This endpoint lets Bart (OpenClaw) send a task, have Jarvis analyze it,
// and then auto-delegate to the right executor.

app.post('/api/jarvis/analyze-and-delegate', async (req, res) => {
    const { task, description, context } = req.body;
    if (!task) return res.status(400).json({ error: 'Missing task' });

    console.log(`\\n[Auto-Delegation] Received task: "${task}"`);

    // Step 1: Ask Jarvis to analyze the task (free, local LLM)
    let jarvisAnalysis = null;
    try {
        const analysisPrompt = `Eres un coordinador de proyectos de software. Analiza esta tarea y responde SOLO con un JSON válido (sin markdown, sin backticks):
{
  "complexity": "low|medium|high",
  "executor": "jarvis|antigravity|claude_code|bart",
...
CONTEXTO: ${context || 'Dashboard de gestión empresarial en React'}`;
        ...
        if (r.ok) {
            ...
            if (jsonMatch) {
                jarvisAnalysis = JSON.parse(jsonMatch[0]);
            }
        }
    } catch (err) {
        console.warn('[Auto-Delegation] Jarvis analysis failed, using fallback:', err.message);
    }

    // Step 2: Fallback analysis if Jarvis couldn't parse
    if (!jarvisAnalysis) {
        const tLower = (task + ' ' + (description || '')).toLowerCase();
        let executor = 'bart';
        let complexity = 'low';

        if (tLower.match(/react|component|frontend|dashboard|ui|css|página|page/)) {
            executor = 'antigravity'; complexity = 'high';
        } else if (tLower.match(/deploy|docker|server|infra|devops|ci\/cd/)) {
            executor = 'claude_code'; complexity = 'high';
        } else if (tLower.match(/analiz|resum|investig|compar|busca/)) {
            executor = 'jarvis'; complexity = 'medium';
        }

        jarvisAnalysis = { complexity, executor, reasoning: 'Fallback keyword analysis', subtasks: [task] };
    }

    console.log(`[Auto-Delegation] Analysis: executor=${jarvisAnalysis.executor}, complexity=${jarvisAnalysis.complexity}`);

    // Step 3: Execute delegation based on analysis
    const db = readDB();
    
    if (jarvisAnalysis.executor === 'antigravity') {
        // Write task file for Antigravity
        const safeName = task.replace(/[^a-zA-Z0-9]/g, '_').substring(0, 30);
        const fileName = `antigravity_task.md`;
        const taskFilePath = path.join(__dirname, fileName);
        const content = `# Tarea Delegada por Jarvis → Antigravity
        
> **Complejidad:** ${jarvisAnalysis.complexity}
> **Análisis de Jarvis:** ${jarvisAnalysis.reasoning}
...
## Descripción
${description || 'Implementar lo solicitado.'}

## Subtareas Sugeridas por Jarvis
${(jarvisAnalysis.subtasks || []).map((s, i) => `${i + 1}. ${s}`).join('\\n')}

## Contexto
- Proyecto: MAIN DASHBOARD GG (React + Vite)
- Estilo: Dark mode, lucide-react icons
- Ruta del proyecto: /Users/gg/MAIN DASHBOARD GG/
`;
        fs.writeFileSync(taskFilePath, content);
        console.log(`[Auto-Delegation] Wrote ${fileName} for Antigravity`);
    }
    ...
    db.activityFeed.unshift({
        id: `act-${Date.now()}`,
        text: `🧠 Jarvis analizó y delegó: "${task}" → ${jarvisAnalysis.executor.toUpperCase()} [${jarvisAnalysis.complexity}]`,
        color: '#06b6d4',
    ...
    res.json({
        ...
        delegatedTo: jarvisAnalysis.executor,
        message: `Tarea analizada por Jarvis y delegada a ${jarvisAnalysis.executor}`
    });
});
...
// ─── LicitIA Integration Bridge ────────────────────────────────

const LICITIA_BASE = path.join(process.env.HOME || '/Users/gg', '.openclaw/workspace/dashboard_licitaciones');
const LICITIA_STATE_FILE = path.join(__dirname, 'licitia-state.json');

function readLicitIAState() {
    try { return JSON.parse(fs.readFileSync(LICITIA_STATE_FILE, 'utf-8')); }
    catch { return { estados: {}, empresas: {}, notas: {} }; }
}
function writeLicitIAState(state) {
    fs.writeFileSync(LICITIA_STATE_FILE, JSON.stringify(state, null, 2));
}

app.get('/api/licitia/feed', (req, res) => {
    try {
        const dataPath = path.join(LICITIA_BASE, 'data.json');
        if (!fs.existsSync(dataPath)) {
            return res.status(404).json({ error: 'LicitIA data.json no encontrado', connected: false });
        }

        const raw = JSON.parse(fs.readFileSync(dataPath, 'utf-8'));
        const licitaciones = raw.licitaciones || [];

        // Load analytics
        let analiticaMap = {};
        try {
            const anPath = path.join(LICITIA_BASE, 'analitica_licitaciones_v2.json');
            const anData = JSON.parse(fs.readFileSync(anPath, 'utf-8'));
            (anData.licitaciones || []).forEach(a => { analiticaMap[a.codigo] = a; });
        } catch {}

        // Load AI summaries
        let resumenMap = {};
        try {
            const rPath = path.join(LICITIA_BASE, 'resumen_ia.json');
            const rData = JSON.parse(fs.readFileSync(rPath, 'utf-8'));
            resumenMap = rData.licitaciones || {};
        } catch {}

        // Load local state (estados, empresa assignments)
        const state = readLicitIAState();

        // Merge everything
        const enriched = licitaciones.map(lic => {
            const an = analiticaMap[lic.codigo] || {};
            const ia = resumenMap[lic.codigo] || null;
            const estado = state.estados?.[lic.codigo] || 'Pendiente';
            const empresaAsignada = state.empresas?.[lic.codigo] || null;

            return {
                ...lic,
                // Analytics
                probabilidad: an.decision?.probabilidad_ganar || 0,
                dificultad: an.decision?.dificultad || 'Baja',
                riesgo: an.decision?.riesgo_sobreprecio || 'Bajo',
                recomendacion: an.decision?.recomendacion || '',
                rubroTecnico: an.rubro_tecnico || '',
                precioRecomendado: an.precio_recomendado || null,
                competencia: an.competencia || null,
                // IA
                resumenIA: ia,
                // State
                estadoManual: estado,
                empresaAsignada,
                nota: state.notas?.[lic.codigo] || '',
            };
        });

        // Filter by empresa if query param provided
        const { empresa, companyId } = req.query;
        let filtered = enriched;
        if (empresa) {
            filtered = enriched.filter(l => l.empresaAsignada && l.empresaAsignada.toLowerCase().includes(empresa.toLowerCase()));
        }
        ...
        console.error('[LicitIA] Error:', err.message);
        res.status(500).json({ error: err.message, connected: false });
    }
});

app.get('/api/licitia/stats', (req, res) => {
    try {
        const dataPath = path.join(LICITIA_BASE, 'data.json');
        if (!fs.existsSync(dataPath)) {
            return res.json({ connected: false, total: 0 });
        }
        ...
        // Load analytics for probability
        let analiticaMap = {};
        try {
            const anPath = path.join(LICITIA_BASE, 'analitica_licitaciones_v2.json');
            const anData = JSON.parse(fs.readFileSync(anPath, 'utf-8'));
        ...
        licitaciones.forEach(l => {
            const est = state.estados?.[l.codigo] || 'Pendiente';
            estados[est] = (estados[est] || 0) + 1;
            pipelineTotal += Number(l.presupuesto_total || l.precio_ref?.toString().replace(/,/g, '') || 0);
            const prob = analiticaMap[l.codigo]?.decision?.probabilidad_ganar || 0;
            if (prob > 0) { probSum += prob; probCount++; }
        });
        ...
        res.json({
            ...
            estados,
            pipelineTotal,
            probPromedio: probCount ? Math.round(probSum / probCount) : 0,
            updated: raw.updated,
        });
    } catch (err) {
        res.json({ connected: false, total: 0, error: err.message });
    }
});

// Update LicitIA state (assign empresa, change estado, etc.)
app.patch('/api/licitia/state/:codigo', (req, res) => {
    const { codigo } = req.params;
    const { estado, empresa, nota } = req.body;
    const state = readLicitIAState();

    if (estado !== undefined) state.estados[codigo] = estado;
    if (empresa !== undefined) state.empresas[codigo] = empresa;
    if (nota !== undefined) state.notas[codigo] = nota;

    writeLicitIAState(state);
    res.json({ success: true, state: { estado: state.estados[codigo], empresa: state.empresas[codigo] } });
});
...
// ─── SuperBrain / NotebookLM Killer Features ─────────────────
...
app.post('/api/brain/obsidian/sync', async (req, res) => {
    try {
        ...
        const vaultPath = req.body.vaultPath || '/Users/gg/Documents/licitia';
        const namespace = req.body.namespace || 'default';
        ...
        // Add to array only if not already exists
        result.documents.forEach(doc => {
            const exists = db.customDocuments.find(d => d.title === doc.title && d.category === 'obsidian');
            if (!exists) {
                db.customDocuments.push({
                    id: doc.id,
                    title: doc.title,
                    content: doc.content.substring(0, 50000), // Cap payload for local size limitations
                    category: 'obsidian',
        ...
        db.activityFeed.unshift({
            id: `act-${Date.now()}`,
            text: `💎 Sincronización de Obsidian completada: ${result.documents.length} notas extraídas de ${req.body.vaultPath || 'licitia'}.`,
            color: '#8b5cf6', // Purple color for Obsidian
            timestamp: new Date().toISOString()
        });
        db.activityFeed = db.activityFeed.slice(0, 50);
        writeDB(db);
        ...
        console.error('[RAG:Obsidian] Error:', err.message);
        res.status(500).json({ error: err.message });
    }
});

app.post('/api/brain/obsidian/export', async (req, res) => {
    try {
        const { type, content, title, vaultPath = '/Users/gg/Documents/licitia' } = req.body;
        ...
        if (type === 'briefing') {
            fileName = `🧠 Guía - ${title || Date.now()}.md`;
            fileContent += `# Guía de Estudio: ${title}\n\n`;
            fileContent += `## 📝 Resumen Ejecutivo\n${content.summary}\n\n`;
            fileContent += `## 🔑 Puntos Clave\n`;
            content.keyPoints.forEach(p => fileContent += `- ${p}\n`);
            fileContent += `\n## ❓ FAQ (Preguntas Frecuentes)\n`;
            content.faq.forEach(q => fileContent += `**Q: ${q.q}**\n> A: ${q.a}\n\n`);
        } else if (type === 'podcast') {
            fileName = `🎙️ Podcast - ${content.title || title || Date.now()}.md`;
            fileContent += `# 🎙️ Podcast: ${content.title || title}\n\n`;
            content.dialogue.forEach(line => {
                fileContent += `**${line.speaker}:** ${line.text}\n\n`;
            });
        } else {
            return res.status(400).json({ error: 'Unsupported type' });
        }

        const fullPath = require('path').join(vaultPath, fileName);
        require('fs').writeFileSync(fullPath, fileContent, 'utf-8');
        ...
        db.activityFeed.unshift({
            id: `act-${Date.now()}`,
            text: `💎 Exportado a Obsidian: "${fileName}"`,
            color: '#8b5cf6',
            timestamp: new Date().toISOString()
        });
        db.activityFeed = db.activityFeed.slice(0, 50);
        ...
        console.error('[RAG:Obsidian:Export]', err);
        res.status(500).json({ error: err.message });
    }
});

app.post('/api/brain/youtube', async (req, res) => {
    try {
        const RAGEngine = app.get('ragEngine');
        if (!RAGEngine) return res.status(503).json({ error: 'RAG engine not available' });
        ...
        const ytDoc = await RAGEngine.indexYouTubeVideo(url, title || 'YouTube Video', category || 'video', namespace);

        // Persist local copy in db.json
        const db = readDB();
        if (!db.customDocuments) db.customDocuments = [];
        db.customDocuments.push({
            id: ytDoc.id,
            title: ytDoc.title,
            content: ytDoc.content.substring(0, 50000), // Cap for local size
            category: ytDoc.category,
            url: url,
            uploadedAt: ytDoc.uploadedAt
        });
        ...
        db.activityFeed.unshift({
            id: `act-${Date.now()}`,
            text: `▶️ Transcrito de YouTube extraído e indexado: "${ytDoc.title}"`,
            color: '#ef4444',
            timestamp: new Date().toISOString()
        });
        db.activityFeed = db.activityFeed.slice(0, 50);
        ...
        res.status(201).json({ success: true, document: ytDoc });
    } catch (err) {
        console.error('[Brain:YouTube] Error:', err.message);
        res.status(500).json({ error: err.message });
    }
});

app.post('/api/brain/podcast', async (req, res) => {
    try {
        const RAGEngine = app.get('ragEngine');
        if (!RAGEngine) return res.status(503).json({ error: 'RAG engine not available' });
        ...
        const db = readDB();
        const doc = (db.customDocuments || []).find(d => d.id === documentId);
        if (!doc) return res.status(404).json({ error: 'Document not found' });

        const script = await RAGEngine.generatePodcastScript(doc.content);
        res.json({ success: true, script });
    } catch (err) {
        console.error('[Brain:Podcast] Error:', err.message);
        res.status(500).json({ error: err.message });
    }
});

app.post('/api/brain/briefing', async (req, res) => {
    try {
        const RAGEngine = app.get('ragEngine');
        if (!RAGEngine) return res.status(503).json({ error: 'RAG engine not available' });
        ...
        const briefing = await RAGEngine.generateBriefing(doc.content);
        res.json({ success: true, briefing });
    } catch (err) {
        console.error('[Brain:Briefing] Error:', err.message);
        res.status(500).json({ error: err.message });
    }
});
